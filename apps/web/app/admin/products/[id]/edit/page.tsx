...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
      ...
      try {
        ...
        if (p) {
          ...
          if (p.tags && Array.isArray(p.tags) && p.tags.length > 0) {
            const parsedTags = p.tags.map((t: any) => (typeof t === 'string' ? t : t.tag?.name || '')).filter(Boolean);
            setTags(parsedTags);
          }

          if (p.manifest && Array.isArray(p.manifest) && p.manifest.length > 0) {
            setManifestRows(
              p.manifest.map((m: any) => ({
          ...
          if (p.variants && p.variants.length > 0) {
            setVariants(
              p.variants.map((v: ApiProductVariant) => ({
                id: v.id,
                name: v.name || '',
                sku: v.sku || '',
                price: v.price ? String(v.price) : '',
                stock: v.stock ? String(v.stock) : '1',
              }))
            );
          }

          if (p.media && p.media.length > 0) {
            setMedia(p.media.map((m: ApiProductMedia) => ({ id: m.id || crypto.randomUUID(), url: m.url })));
          }
        }
      } catch (err: unknown) {
  ...
  const handleSubmit = async (e: React.FormEvent, saveStatus: string) => {
    ...
    try {
      // 1. Upload any pending new image files to Cloudflare R2 via backend
      ...
      alert(`Pallet updated as "${saveStatus}" successfully!`);
      router.push('/admin/products');
    } catch (err: unknown) {
      alert(`Error updating pallet: ${err instanceof Error ? err.message : String(err)}`);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (error) {
    return (
      <div className="text-center py-20 bg-white border border-neutral-200 rounded-2xl shadow-sm max-w-lg mx-auto">
        ...
        <p className="text-sm text-rose-500 max-w-sm mx-auto mb-6">{error}</p>
        <Link
          href="/admin/products"
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-2xl font-bold tracking-tight text-neutral-900">Edit Pallet</h2>
              <span
            ...
            <p className="text-neutral-500 mt-0.5 text-sm">Update manifest, condition, pricing, variants, and visibility.</p>
          </div>
        </div>
        ...
        <div className="hidden sm:flex items-center gap-2">
          <button
            ...
            className="px-4 py-2.5 rounded-xl border border-rose-200 text-sm font-semibold text-rose-600 hover:bg-rose-50 transition-all disabled:opacity-50"
          >
          ...
          <Section title="Basic Information" description="Name, description, condition, and categorisation." icon={'fi fi-rr-document'}>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="Category" required hint="Categorisation for catalog browsing.">
                ...
                <Field label="Pallet Condition" required hint="Condition grade of inventory in this pallet.">
                  <div className="relative">
                    <select
                      id="condition-select"
                      value={condition}
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col gap-2 sm:hidden">
            ...
            <button
              ...
              onClick={handleDeleteProduct}
              disabled={submitting || deleting}
              className="w-full py-3 border border-rose-200 text-rose-600 rounded-xl font-bold text-sm hover:bg-rose-50 disabled:opacity-50 transition-colors"
            >
              {deleting ? 'Deleting…' : 'Delete Pallet'}
            </button>
          </div>
        </div>
      </form>
